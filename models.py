...
class Generator(nn.Module):
    ...
    def forward(self, x, output_emb_1, output_emb_2, output_emb_3, output_emb_4, output_emb_5):
        ...
        x = self.downsamplinglayer_1(x)
        x = self.downsamplinglayer_2(x)
        x = self.downsamplinglayer_3(x)
        x = self.downsamplinglayer_4(x)
        x = self.downsamplinglayer_5(x)

        x = self.spade_1(x, output_emb_5)
        x = self.noise_1(x)
