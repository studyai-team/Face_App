...
class InferenceWrapper(nn.Module):
    ...
    def preprocess_data(self, input_imgs, crop_data=True):
        ...
        return poses, imgs, segs, stickmen, [center[0]-size, center[1]-size, center[0]+size, center[1]+size]
    ...
    def forward(self, data_dict, crop_data=True, no_grad=True):
        ...
        (source_poses, 
         ...
         source_segs, 
         source_stickmen,
         source_pos) = self.preprocess_data(data_dict['source_imgs'], crop_data)

        (target_poses,
         target_imgs, 
         target_segs, 
         target_stickmen,
         target_pos) = self.preprocess_data(data_dict['target_imgs'], crop_data)
        ...
        data_dict["source_pos"] = source_pos
        data_dict["target_pos"] = target_pos
